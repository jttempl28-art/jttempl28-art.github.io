...
def redact_text(text):
    ...
    # Name redaction: replace capitalized words
...
async def process_files(*args):
    ...
    files = Element("file-upload").element.files
    problem = Element("problem").value
    if not api_key or not files or not problem:
...
# Bind button
Element("process-btn").element.addEventListener("click", process_files)
